...
const startServer = async () => {
  ...
  app.use((req, res) => {
    ...
    if (!isAuthenticated(req) && req.path !== '/' && !req.path.startsWith('/login') && !req.path.startsWith('/register')) {
      res.redirect('/');
      return;
    }

    // If user is authenticated and on landing/login/register, redirect to dashboard
    if (isAuthenticated(req) && (req.path === '/' || req.path.startsWith('/login') || req.path.startsWith('/register'))) {
      res.redirect('/c/new');
      return;
    }
